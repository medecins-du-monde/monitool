...
angular
	...
	.directive('pdfExportLogicalFrame', function($filter) {
		var translate = $filter('translate');

		var computeSources = function(project, indicators) {
			var sources = {};
				...
				if (indicator.computation) {
					for (var key in indicator.computation.parameters) {
						var elementId = indicator.computation.parameters[key].elementId,
							form = project.forms.find(function(form) {
								return !!form.elements.find(function(element) { return element.id === elementId; });
							}),
							element = form.elements.find(function(element) { return element.id === elementId; });
		...
		var logicalFrameToDocDefinition = function(project, logicalFrame, format) {
			var doc = angular.copy(BASE_DOCUMENT);
			doc.pageSize = "A4";
			doc.pageOrientation = format;

			doc.content = [
				{text: logicalFrame.name, style: 'header3'}
			];
			...
			var table = {
				...
				body: [
					[
						{text: translate('project.intervention_logic'), style: "bold"},
						{text: translate('shared.indicators'), style: "bold"},
						{text: translate('project.collection_form_list'), style: "bold"},
						{text: translate('project.assumptions'), style: "bold"}
					]
				]
			};
			...
			table.body.push([
				[
					{text: translate('project.goal'), style: "bold"},
					{text: logicalFrame.goal, style: 'normal'}
				],
				{ul: logicalFrame.indicators.map(function(i) { return i.display; }), style: 'normal'},
				{ul: computeSources(project, logicalFrame.indicators), style: 'normal'},
				""
			]);
			...
			logicalFrame.purposes.forEach(function(purpose, purposeIndex) {
				table.body.push([
					[
						{text: translate('project.purpose') + (logicalFrame.purposes.length > 1 ? " " + (purposeIndex + 1) : ""), style: "bold"},
						{text: purpose.description, style: 'normal'}
					],
					{ul: purpose.indicators.map(function(i) { return i.display; }), style: 'normal'},
					{ul: computeSources(project, purpose.indicators), style: 'normal'},
			...
			logicalFrame.purposes.forEach(function(purpose, purposeIndex) {
				purpose.outputs.forEach(function(output, outputIndex) {
					table.body.push([
						[
							{text: translate('project.output') + " " + (logicalFrame.purposes.length > 1 ? " " + (purposeIndex + 1) + '.' : "") + (outputIndex + 1), style: "bold"},
							{text: output.description, style: 'normal'}
						],
						{ul: output.indicators.map(function(i) { return i.display; }), style: 'normal'},
						{ul: computeSources(project, output.indicators), style: 'normal'},
		...
		return {
			...
			scope: {
				project: '=project',
				logFrame: '=pdfExportLogicalFrame'
			},
			link: function(scope, element, attributes) {
				element.bind('click', function(e) {
					var docDef = logicalFrameToDocDefinition(scope.project, scope.logFrame, attributes.format || 'portrait');
